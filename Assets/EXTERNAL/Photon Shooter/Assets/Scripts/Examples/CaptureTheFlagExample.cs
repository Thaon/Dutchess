...
/// <summary>
/// Extended class to handle the Capture the flag Game Mode.
/// </summary>
/// 

public class CaptureTheFlagExample : GameMode
{
    ...
    public override void Awake()
    {
        ...
        {
            if(GameObject.FindObjectOfType(typeof(Flag)) == null)
                PhotonNetwork.InstantiateSceneObject("Flag", GetRandomSpawnPoint(), Quaternion.identity, 0, null);
        }
    }
    ...
    /// Then we instance the player again. If we had the flag then we detach it from the player. </summary>
    public override void OnDeath(GameObject player, PhotonPlayer other)
    {
        ...
        {
            flag.DetachFlag();
            flag.GetComponent<PhotonView>().RPC("DetachFlagRPC", PhotonTargets.Others);
        }
